...
Statut : les quatre lots sont implémentés ; restent les vérifications en navigateur et une migration de données (voir la ligne de statut en fin de document)
...
| D — page article | Implémenté, à vérifier en dev | | Nouvelle route `routes/content/blog/[id]/page.tsx` : deux colonnes (`lg:grid-cols-[1fr_320px]`), corps à gauche, panneaux « Publication » / « Classement » / « Adresse » / « Image » à droite ; garde-fou `Prompt` sur « Retour à la liste » quand le formulaire est modifié ; « Voir sur le site » désactivé avec `Tooltip` tant que l'article n'est pas publié **ou** que des modifications ne sont pas enregistrées. `CrudResource` gagne les props `editHref`, `onCreate` et `createPending`, et exporte `FieldInput`. La dérivation du slug est extraite de `FormBody` en hook exporté `useDerivedFields` : sur la page article, le titre et le slug sont dans deux blocs différents, il leur faut un seul gestionnaire partagé. Les définitions de champs du blog sont extraites dans `src/admin/lib/blog-fields.ts`, volontairement hors de `routes/` pour ne pas interférer avec le scanner de routes de l'admin. **Écart 1 (simplification)** : le `Drawer` d'édition et le `FocusModal` de création restent montés au lieu d'être conditionnés ; leurs déclencheurs étant remplacés, ils ne s'ouvrent jamais, et Radix ne rend pas leurs enfants tant qu'ils sont fermés — aucun second éditeur Tiptap n'est donc instancié. **Écart 2 (meilleure solution que prévue)** : Medusa 2.15.5 expose déjà l'option native `admin.storefrontUrl`, que son bundler injecte dans le bundle sous la forme du global `__STOREFRONT_URL__`. Elle remplace le `admin.vite` → `define` maison prévu au plan. Vérifié dans le bundle produit : `localhost:4321` est inliné et il ne reste aucune occurrence du global brut. Comme la valeur est lue au moment du build, `apps/backend/Dockerfile` déclare un `ARG STOREFRONT_URL`, et la variable est documentée dans `.env.template` et `DEPLOY.md`. Preuves : `tsc --noEmit` propre, `medusa build` OK (backend 1,99 s, admin 10,94 s), et la route `content/blog/:id` est bien présente dans le bundle. Vérifications 1 à 6 du lot non faites : elles demandent un navigateur. |
